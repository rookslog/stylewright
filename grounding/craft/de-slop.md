...
| E-09 | **One idea.** A triad where two items would do buys a clause and adds no idea. So does a "not just X but Y" frame whose Y restates X. "Not just a parser bug but a design problem" promises two findings and delivers one. | What a departure looks like |  |  | Our own guidance, the departure from E-02, and the one worked example here |  |
